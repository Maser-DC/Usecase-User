...
func (interactor *UserInteractor) ChangeFirstName(userID string, FirstName string) error {
	user, err := interactor.UserRepository.FindById(userID)
	user.FirstName = FirstName
...
func (interactor *UserInteractor) ChangeLastName(userID string, LastName string) error {
	user, err := interactor.UserRepository.FindById(userID)
	user.LastName = LastName
...
func (interactor *UserInteractor) ChangeEmail(userID string, email string) error {
	user, err := interactor.UserRepository.FindById(userID)
	user.Email = email
	err = interactor.UserRepository.Update(user)
	return err
